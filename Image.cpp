...
void Image::Fade(float u) {
        for (int j = 0; j < 1048576; j++) {
            data[j].r = floor(data[j].r * u);
            //std::cout << floor(data[j].r) << " ";
            data[j].g = floor(data[j].g * u);
            data[j].b = floor(data[j].b * u);
            //PutPixel(j / 1024, j % 1024, image.GetPixel(j / 1024, j % 1024));
        }
        u -= 0.05;
...
void Image::Copy(Image &image) {
    for (int j = 0; j < 1048576; j++) {
        data[j].r = image.data[j].r;
        data[j].g = image.data[j].g;
        data[j].b = image.data[j].b;
        data[j].a = image.data[j].a;
    }
}
int Image::Get() {
    return data[10].r;
...
void Image::drawTile1(Image image) {
    for (int i=0; i<1024; i++){
        for (int j=0;j<1024; j++){
            PutPixel(i, j, image.GetPixel(i, j));
        }
    }
}
Image::~Image()
